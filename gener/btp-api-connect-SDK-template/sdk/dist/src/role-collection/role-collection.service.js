...
let RoleCollectionService = class RoleCollectionService {
    ...
    async getRoleCollections(req, res) {
        const collection = await role_collections_api_1.RoleCollectionsApi.getRoleCollections({
            showUsers: true
        }).execute(this.destination);
        return res.send(collection);
    }
    async changeRoleCollectionDescription(req, res) {
        const collection = await role_collections_api_1.RoleCollectionsApi.changeRoleCollectionDescription(req.body.id, req.body.collection).execute(this.destination);
        return res.send(collection);
    }
    ...
    async deleteRoleFromRoleCollection(req, res) {
        let body = req.body;
        const collection = await role_collections_api_1.RoleCollectionsApi.deleteRoleFromRoleCollection(body.roleCollectionName, body.roleName, body.roleTemplateAppId, body.roleTemplateName).execute(this.destination);
        return res.send(collection);
    }
};
__decorate([
